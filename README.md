...
This project and proof of concept demonstrates how we could better manage end-to-end the migration process of a Java application from by example Spring Boot to Quarkus using new concepts able to perform the process using ordered instructions managed by different providers.
...
While such limitations are not problematic for the users doing manually the transformation, that will become a real challenge when it is needed to apply recipes or delegate to AI the responsibility to propose solutions. Ai will generate hallucinating results and the lack of predictability will discourage many users to rely on it. On the other site, as openrewrite during the execution of a recipe will compile the code, then the user will be faced to compilation errors, etc.
