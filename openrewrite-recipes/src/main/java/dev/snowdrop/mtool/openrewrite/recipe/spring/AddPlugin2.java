package dev.snowdrop.mtool.openrewrite.recipe.spring;
...
public class AddPlugin2 extends Recipe {
	...
	@Option(displayName = "File pattern", description = "A glob expression that can be used to constrain which directories or source files should be searched. "
	...
	public TreeVisitor<?, ExecutionContext> getVisitor() {
		return new AddPlugin2.AddPluginVisitor();
	}
	...
	private class AddPluginVisitor extends MavenIsoVisitor<ExecutionContext> {
		...
		public Xml.Document visitDocument(Xml.Document document, ExecutionContext ctx) {
			...
			if (!root.getChild("build").isPresent()) {
				document = (Xml.Document) new AddToTagVisitor<>(root, Xml.Tag.build("<build/>")).visitNonNull(document,
		...
		public Xml.Tag visitTag(Xml.Tag tag, ExecutionContext ctx) {
			...
			if (BUILD_MATCHER.matches(getCursor())) {
				...
				if (maybePlugins.isPresent()) {
					...
					t = (Xml.Tag) new AddToTagVisitor<>(t, Xml.Tag.build("<plugins/>")).visitNonNull(t, ctx,
				...
				if (maybePlugin.isPresent()) {
					...
					if (version != null && !version.equals(plugin.getChildValue("version").orElse(null))) {
						if (plugin.getChild("version").isPresent()) {
							t = (Xml.Tag) new ChangeTagValueVisitor<>(plugin.getChild("version").get(), version)
					...
					Xml.Tag pluginTag = Xml.Tag.build("<plugin>\n" + "<groupId>" + groupId + "</groupId>\n"
							...
							+ (extensions != null ? extensions.trim() + "\n" : "")
							+ (configuration != null ? configuration.trim() + "\n" : "")
							+ (dependencies != null ? dependencies.trim() + "\n" : "") + "</plugin>");
					t = (Xml.Tag) new AddToTagVisitor<>(plugins, pluginTag).visitNonNull(t, ctx,
							getCursor().getParentOrThrow());
				}
			}
